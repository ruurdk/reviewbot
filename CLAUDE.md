...
**Built and verified against the live services** (steps 1–7): the shared harness, both agents, the primer, the Agent Memory client, quality scoring, the sequence runner, the frozen 19-PR dataset with 7 gold-labelled PRs, the provisioned store, and the replay page — 255 tests, none of which need network or credentials.

**Not built:** `review_policy` memories, i.e. procedural memory (the spec sequences it last, and nothing depends on it). Memory invalidation for the convention-change beat is also absent — spec §6 says to keep that PR and disclose the gap rather than drop it. **Consolidation** (merging N findings into one via a model call) is also absent and is a separate, output-token-costing optimization — output is the expensive half of the bill (spec §7g).

**Built but do not trust it yet: dedup-on-write** (`--dedupe-writes`, off by default). It fired **once in 101 writes** in run-2, because the finding id keys on `(module, topic)` and `topic` is model-generated free text — the same concept on the same file came back as `...python-3-9-compatibi...` and `...python-version-compa...`. So the loop it exists to break survived: the memory agent restated one false convention 9 times across 7 PRs in run-2 against 11 across 9 in run-1, while the baseline said it twice. This is the *same* mistake as the original unroutable-module bug (spec §7f): free text from a model is not a stable key. Fixing it means a semantic search over `review_finding` scoped to the module before writing, merging above a threshold — cheap, since searches are `billable=False` (one round trip, zero model tokens). A coarser key built from more model text will not work: `finding_class` is free text too. Consequence to know before building on it: with one merge per run, `occurrences` is 1 almost everywhere, so the recurrence signal `review_policy` was meant to consume is not being produced yet. It also stays off by default because it changes the finding id scheme, and reproducing run-1 means reproducing its ids.

**Two complete runs, both frozen.** `runs/frozen/run-1` (pooled retrieval, append-only writes) and `runs/frozen/run-2` (`--retrieval-split conv=10,find=10 --dedupe-writes`), 19/19 PRs each under the same fingerprint `bd49dcd2fae0fe0d`. Freezing is a convention, not a code path: `cp -r runs/<id> runs/frozen/<id>` then `report runs/frozen/<id>` so the frozen summary is what current analysis code produces. `runs/run-1` and `runs/run-2` remain as the working directories a resume could still append to. **The headline is the marginal per-review saving** (+27.7% run-1, +31.6% run-2), not the cumulative one, which is a function of sequence length. The gold labels are still `CANDIDATE` — written by Claude, which is the model family under evaluation.

**What run-2 settled.** Splitting the retrieval budget answered the question run-1's ledger could not: conventions returned their full 10 on all 19 PRs while findings started at 0 and only reached their cap from PR 7, so in a pooled window it is conventions that crowd. Precision went 19% → 25% on *fewer* retrieved records (316 against 380). And the control held: both runs' baselines read byte-identical context (3,890,269 tokens) yet billed $32.67 against $33.54, which is the noise floor for any single-run comparison — including the quality panel, where the baseline's labelled-defect count moved 1 → 2 between runs on identical input.
...
Full operational detail — setup, credentials, run safety, the sandbox caveat, and the live-service findings — is in [docs/operations.md](docs/operations.md). The essentials:
...
| [reviewbot/accounting.py](reviewbot/accounting.py) | `Usage.context_volume` is the *sum* of the three input fields, so `input_tokens` can't be reported as prompt size. `latency_ms` covers draining a streamed body and `ttfb_ms` is the headers-only figure — they differ by *minutes* on a long review, so reporting time-to-first-byte as latency makes every streamed call look instant. **Rows in `runs/run-1` predate this fix and carry TTFB in `latency_ms`; they have no `ttfb_ms` at all, which is how to spot them.** `CallRecord` requires `{agent, pr_id, pr_ordinal, phase}`. Search rows carry `retrieved_ids` — *which* records filled the window, not just how many; a count cannot say whether a saturated window is filling with conventions or findings, and run-1's rows have `retrieved_ids: null`, which is how to spot them. Memory ops are `billable=False` with `injected_tokens` as attribution only — adding them to a total double-counts. Ledger is append-only JSONL and resumes `seq`. |
...
| [reviewbot/analysis.py](reviewbot/analysis.py) | The §7d repricing rule, stated in code: a cache read whose prefix was written under a *different* PR ordinal is charged at 1.0x, applied to **both** agents. `breakeven_ordinal()` ignores a crossing that reverses. `cache_integrity()` is the pre-flight before trusting any cost number, and it compares the gap between two calls against the TTL — an expired entry is not an unstable prefix, and calling it one sends a reader hunting a nondeterminism bug that is not there. `retrieval_mix()` attributes each retrieval window by memory type from the id prefix, and reports `instrumented: false` for a run that predates id logging rather than an empty mix that reads as "retrieval returned nothing". |
| [reviewbot/review.py](reviewbot/review.py) | **One** frozen `SYSTEM_PROMPT` for both agents (reproduced in [docs/prompts.md](docs/prompts.md), generated by `tools/dump_prompts.py`; `tests/test_prompts.py` fails if it drifts) — it names the optional "Prior knowledge" section so its bytes don't change when memories are absent. Stable blocks (conventions) precede the breakpoint; per-PR source, memories, and the diff follow it. `Finding`/`FINDINGS_SCHEMA` make gold-set comparison mechanical, and `memories_used` is the retrieval-precision signal. |
...
| [reviewbot/agents.py](reviewbot/agents.py) | **A model-generated `module` is resolved against the PR's touched files before it becomes a topic** (`memory.resolve_modules`): exact path, then unique basename, then every touched file under a named directory. Free text from a model is not a path — on the first real run 5 of 6 written findings named a directory, wrote successfully, and were unretrievable forever. An ambiguous basename resolves to *nothing* rather than guessing, and anything unresolvable is dropped **and logged** as lost recall. The only difference between the agents is `volatile_blocks`. Retrieval is **pooled by default** (one search over both memory types, sharing `--retrieval-limit`) so run-1 stays reproducible; `--retrieval-split conv=10,find=10` issues one search per type with its own budget, which costs an extra round trip and zero model tokens because searches are `billable=False`. That matters because run-1's pooled window came back *full* on all 19 PRs, so conventions and a monotonically growing pile of findings compete for fixed slots (spec §7f). The primer runs one call per module (so re-priming one changed module doesn't re-pay for the rest) at `pr_ordinal=0`, and writes with deterministic ids for idempotence. Write-phase distillation is a measured model call; `distill_writes=False` gives a zero-model-token variant. `dedupe_writes=True` makes the finding id a pure function of (module, topic) — which is what turns it into a dedup key detectable with a GET rather than a similarity search — and merges a repeat via the guarded `patch_fields`, unioning topics so a recurrence in a new file cannot make the record unreachable from the old one. It bumps `occurrences` and `last_pr_ordinal` while `pr_ordinal` keeps its first-seen value, so chronology still means what it says. **Attributes come back from the service as strings** (`4` reads as `"004"`), and re-encoding that string is a silent no-op — so the read-modify-write goes through `decode_ordinal_attr`, without which `occurrences` arithmetic gets a str. Chronology rides in `attributes.pr_ordinal` because `createdAt` is server-assigned. |
| [reviewbot/memory.py](reviewbot/memory.py) | Enforces the four Agent Memory constraints that silently corrupt runs: the `^[a-zA-Z0-9-]+$` id/namespace pattern, bulk-create `errors` (a 200 can still mean half the writes failed), `filterOp: all` + `topics: {in: […]}` for module retrieval (attribute clauses have no membership operator), and `eq`-not-`ne` namespace isolation. `wait_for_visibility()` polls for eventual consistency and logs the wait as excluded from latency. Every id it mints is prefixed with its type's `ID_PREFIX` tag, because a search response has no `memoryType` and no `attributes` — the id is the only self-describing thing a retrieved record has, so `memory_type_of()` is what makes retrieval attributable without a GET per record. |
| [reviewbot/quality.py](reviewbot/quality.py) | Two scores that are never averaged. Gold labels carry `must_not_flag` items, which is what makes the false-positive trap measurable. Blind PRs (no human comments) are excluded from proxy ratios rather than scored zero. **The trap score is positional and that is currently too weak to trust**: matching is file plus a ±5-line window, so a trap labelled with no `line` degrades to "did the agent mention this file at all". Both traps run-2 scored against the memory agent are that case — a `line: null` nit on #4131 and an unrelated bookkeeping finding at line 380 on #4177, neither of them the labelled pattern. Before quoting a trap number, give the trap labels lines or match on text. |
...
The smoke test renders **three** times: once with `fetch` rejecting (the synthetic fallback and its banner), once against `web/scripts/fixtures/report-real.json` — a report the *harness* produced, regenerated with `python3 tools/make_page_fixture.py runs/<id>` — and once in multi-run mode, where the page fetches `runs.json` and then one report per run listed in it. `runs.json` is hand-maintained (nothing generates it) and its `url` per run must exist both in `web/public/` and, for the smoke test, in `web/scripts/fixtures/`. The fetch stub is URL-aware on purpose: one that ignored its argument would serve the manifest as a report and every multi-run check would pass on nonsense. Covering only the synthetic path is how `report.per_pr` came to be missing from the harness while every check passed; a real run would have rendered a broken page. It also clicks through all four acts, because an act renders only while selected, so a broken chart or table in acts 2–4 is otherwise invisible.
