...
### 7f. The retrieval window is fixed while the store grows — measured across two runs, and what dedup-on-write got wrong

Sequential PRs mean the store only ever gets bigger. Nothing in the harness supersedes or expires a memory, and the one mechanism that merges them — dedup-on-write — turned out to fire almost never, which is the subject of the second half of this section. Measured over two full 19-PR runs:

| | run-1 (pooled retrieval, append-only) | run-2 (split retrieval, `--dedupe-writes`) |
|---|---|---|
| after the primer | 102 `repo_convention` | 100 `repo_convention` |
| after 19 PRs | 198 — 96 `review_finding` appended, 0 merged, 0 superseded | 200 — 100 `review_finding` records from 101 writes, **1** merged, 0 superseded |

The obvious worry is that per-PR retrieval cost therefore grows with sequence position, eating the saving that §7d and §7e work so hard to price honestly. **It does not, and the reason matters more than the reassurance.** Retrieval ran with `limit: 20` and returned *exactly* 20 records on every PR from the first one onward. Injected memory tokens are consequently flat at ~3,705 per PR — **70,397 tokens across the sequence, 3.7% of the memory agent's total input.** Store growth is absorbed by truncation, not by the bill. Run-2, with the budget split ten and ten, injected *less*: 60,247 tokens, 3.2%. The line item is small and constant under both retrieval shapes.
...
- **Retrieval precision is 19%** in run-1 — 74 of 380 retrieved memories were ever cited, per-PR between 0.05 and 0.35, with no upward trend. The accumulating records are not earning their slots. This is precisely the failure mode §7b names: retrieving many to use few. Run-2 raised it to **25%** (80 of 316) by retrieving *fewer* records, which is the shape of a fix worth having: the win came from composition, not volume.

**The measurement run-1 could not make, made.** `log_op` recorded `returned: <count>` and not *which* records came back, so the question that decides whether compaction is worth building — are the primed conventions crowding out the episodic findings? — was unanswerable from run-1's ledger. The harness now logs retrieved ids and `analysis.retrieval_mix()` attributes each window by memory type (run-1's rows predate this and report `instrumented: false` rather than an empty mix, because an empty mix would read as "retrieval returned nothing", which is the opposite of what happened).

Run-2's answer is lopsided. With `conv=10,find=10`, **conventions returned their full 10 on all 19 PRs**, while findings started at 0, first filled their own budget at PR 7, and totalled 126 against the conventions' 190. Conventions never run short of above-threshold candidates; findings frequently do. So in a *pooled* window, where the two compete for the same 20 slots, the crowding direction is settled — conventions crowd findings, not the reverse. What remains unmeasurable is run-1's own mix: its rows carry no ids, so how skewed its 380 retrievals actually were cannot be recovered, only inferred from this.

**Where compaction does belong, in two distinct roles.** They are worth separating because only one of them is about tokens:

1. **Dedup-on-write — built behind `--dedupe-writes`, and as keyed it does not work. A correctness fix that has not yet landed.** An append-only store is what turns a wrong belief into a growing one. The clearest instance in run-1: a false claim that redis-py's PEP 604 unions need `from __future__ import annotations` (untrue — `requires-python = ">=3.10"`) was restated **11 times across 9 PRs** by the memory agent versus **3 across 3** by the baseline, and persisted as **10 separate records**. The loop is mechanical: the claim sits in the retrieval window → the model restates it → the write phase persists it again under a new id → it now has more copies competing for the window.

    Run-2 turned the flag on and the loop barely moved: **9 restatements across 7 PRs, persisted as 9 separate records** (baseline: 2). Across the whole run, **1 write in 101 merged**. So the earlier draft of this section was wrong to assert that merging via the guarded `PATCH .../fields` *breaks* the loop; what it breaks is a narrower thing, and only when the key collides.

    **Why it does not collide.** The finding id is a pure function of (module, topic) instead of (module, topic, ordinal), which was supposed to turn the id itself into the dedup key — **no similarity search and no fuzzy matching**. That property, presented here as a virtue, is the defect: `topic` is model-generated free text, and the model does not phrase a concept identically twice. Run-2 wrote the same claim about the same file as `...commands-helpers-py-python-3-9-compatibi...` on PR 5 and `...commands-helpers-py-python-version-compa...` on PR 9. This is the *same* error as the unroutable-module bug (§4e): free text from a model is not a stable key. Nor is a coarser key made of more model text a fix — `finding_class` is free text too (one real value: `"update_*_config / setter paths skip the guards enforced in __init__"`).

    **The write path itself is correct, and that is worth separating from the key.** The one merge that did fire is verifiable in the store: record `find-redis-asyncio-connection-py-runtime-reconfigur-76e0e6e98195` in namespace `redis-py-run-2` carries `occurrences: 2`, `last_pr_ordinal: 17`, and `pr_ordinal: 15` — recurrence explicit, first-seen chronology preserved, `updated_at` after `created_at`. The guarded PATCH, the string-attribute decoding, and the chronology rule all behave as specified. Only the key is broken. Note the consequence for what comes next, though: with one merge in a run, `occurrences` is 1 almost everywhere, so the recurrence signal `review_policy` was meant to consume **is not being produced yet**.

    **The fix, and why it is cheap.** Before writing a finding, run a semantic search over `review_finding` scoped to that module and merge into any hit above a threshold, instead of trusting the id to collide. A search is `billable=False` — one extra round trip and **zero model tokens** — so unlike consolidation it never touches the output side of the bill (§7g). This is the one place the "no similarity search" stance in this section should be reversed; it was reasoning about implementation simplicity in a place where only key stability matters.

    **And even a working dedup does not make a false claim less likely to be restated** — run-2 is direct evidence: the copies and the restatements are separate problems, and it is the restatements that reach the review. Suppressing a refuted claim is `review_policy`'s job, still unbuilt. This is the same gap `review_policy` (§4d) and memory invalidation (§6) address from the other side.
...
**The cheapest experiment needed no compaction at all, and it has now been run.** Splitting the single pooled search into one search per memory type with its own budget (`--retrieval-split conv=10,find=10`) costs two round trips and zero model tokens — searches are `billable=False`, so the knob moves the retrieval mix without touching the cost comparison. Over the full sequence it moved the per-review saving from **+27.7% to +31.6%**, raised precision from 19% to 25%, and did something more interesting to the worst case: PR #4131, where run-1's memory agent cost **46.8% more** than no memory at all, came out **6.8% cheaper** in run-2. That PR gives the baseline unusually little to re-derive, so it is exactly where injected context has to earn its slots, and capping conventions at 10 rather than letting them take all 20 is the plausible mechanism.

    Two caveats keep this suggestive rather than attributed. Run-2 changed **two** knobs at once (retrieval shape *and* `--dedupe-writes`), and generation is nondeterministic: both runs' baselines read byte-identical context — 3,890,269 tokens, which is the control working — and still billed $32.67 against $33.54. A 3.9-point difference sits close to that noise. Isolating the retrieval shape needs a run with dedup off, which is also why pooled remains the default: run-1 has to stay reproducible.
